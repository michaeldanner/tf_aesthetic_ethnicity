...
def get_model(cfg):
    ...
    pred_ethnic = Dense(units=2, activation="softmax", name="pred_ethnic")(features)
    pred_attractive = Dense(units=101, activation="softmax", name="pred_attractive")(features)
    model = Model(inputs=base_model.input, outputs=[pred_ethnic, pred_attractive])
    return model
